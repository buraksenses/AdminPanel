...
export const addConfiguration = async (
  ...
  try {
    ...
    if (response.data.statusCode === 201) {
      ...
      showSuccessToast(
        `${BuildingTypeLabels[buildingType]} configuration added! Values: cost:${buildingCost} , constructionTime:${constructionTime}`
      );
      reset();
    } else {
    ...
    if (checkSessionExpired(error)) return;
...
export const removeConfiguration = async (
  ...
  try {
    ...
    const response = await apiClient.delete(
      `${writeBaseURL}/api/buildings/${configToRemove.id}`
    );
...
export const getConfigurations = async (
  ...
  try {
    ...
    const updatedConfigurations = data.data.map((config) => ({
      ...config,
      buildingType:
        BuildingTypeStringToInt[config.buildingType] || config.buildingType,
    }));
